...
package single_instance_with_kms
...
func TestExampleWithKms(t *testing.T) {
	...
	example.DefineVerify(func(assert *assert.Assertions) {
		...
		KmsKeyRingId := example.GetStringOutput("kms_keyring_id")
		KmskeyId := example.GetStringOutput("kms_key_id")
		SchedulerRegion := "us-central1"
		kmsRegion := "europe-west1"
		...
		kmsKeys := gcloud.Run(
			t, "kms keys list --location "+kmsRegion+" --keyring "+KmsKeyRingId,
			gcloud.WithCommonArgs([]string{"--project", projectID, "--format", "json"})).Array()

		kmsKeyVersions := gcloud.Run(
			t, "kms keys versions list --location "+kmsRegion+" --keyring "+KmsKeyRingId+" --key "+KmskeyId,
			gcloud.WithCommonArgs([]string{"--project", projectID, "--format", "json"})).Array()
		...
		kmsKeyMatch := utils.GetFirstMatchResult(
			t, kmsKeys, "name", KmskeyId,
		)
		...
		assert.Equal("2000", spannerInstanceMatch.Get("processingUnits").String(), "Spanner Instance processingUnits should be 200")
		assert.Equal(
		...
		assert.Equal(
			...
			"Spanner DB encryption type should be CUSTOMER_MANAGED_ENCRYPTION",
		)

		assert.Equal(
			kmsKeyVersions[0].Get("name").String(),
			spannerDbMatch.Get("encryptionInfo").Array()[0].Get("kmsKeyVersion").String(),
			"Spanner DB encryption Key  should be "+kmsKeyVersions[0].Get("name").String(),
		)
		...
		assert.Contains(
			"ENABLED",
			kmsKeyMatch.Get("primary.state").String(), "KMS Key should be ENABLED",
		)
	})
	example.Test()
}
